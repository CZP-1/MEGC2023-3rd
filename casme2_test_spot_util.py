...
def crop_picture(img_rd,size):
    ...
    if not len(faces) ==0 :
        ...
        img_rd=cv2.imread("/home/data/CZP/MEGC/MEGC/data/test_set/CAS/dzw_f3/0009.jpg")
        img_gray = cv2.cvtColor(img_rd, cv2.COLOR_RGB2GRAY)
        faces = detector(img_gray, 0)
        for i in range(len(faces)):
        #取特征点坐标
...
def fenxi1(flow_total,imf_sum1,yuzhi1,yuzhi2): #使用寻找峰的方法
    ...
    for i in range(len(flow_total_pp)): #第二次筛选
        ...
        for j in range(start,end):
            ...
            if (flow_total[j] - low > yuzhi2 and imf_sum1[j] - low1 >0.5):
            # if (flow_total[j] - low > yuzhi2):
                flow_total_fenxi.append(j)
    flow_total_pp2 = []
    if (len(flow_total_fenxi) > 0):
...
def proce2(flow1_total,yuzhi1,yuzhi2,position,xuhao,k,a,totalflow,totalflow_mic,totalflow_mac):
    ...
    position=img_save+'/'+position+str(xuhao)+'_'

    threshold_filt = 2
    flow1_total_edm1 = sim_filter.filt(flow1_total[a:-a], 1, threshold_filt, 30)  # 滤波
    # flow1_total_edm1 = flow1_total[a:-a] #不使用滤波
    hh = len(flow1_total_edm1)+2 #作用等同于200
    
    # yuzhi1=np.mean(flow1_total_edm1)
    # yuzhi1=np.max(flow1_total_edm1)*0.75
    # yuzhi2=(np.max(flow1_total_edm1)-yuzhi1)*0.5
    ...
    # flow1_total_fenxi = fenxi1(flow1_total_edm2,flow1_total_edm2,yuzhi1,yuzhi2) 
    ...
    global start_1
    global end_1
    draw_img(flow1_total[a:-a],position,'flow')
    draw_img(flow1_total_edm1,position,'filt_5')
    draw_img(flow1_total_edm2,position,'first')
    draw_img(imf_sum1,position,'last')
    # x = np.arange(len(flow1_total[a:-a])) # 创建x的取值范围
    # plt.clf()
    # plt.plot(x, flow1_total[a:-a], label='flow') 
    # plt.plot(x, flow1_total_edm1, label='filt_5') 
    # plt.plot(x, flow1_total_edm2, label='first') 
    # plt.plot(x, imf_sum1, label='last') 
    # plt.legend() #自动检测要在图例中显示的元素，并且显示
    # plt.savefig(position+'all'+'_'+str(start_1)+'_'+str(end_1)+'.png')

    return totalflow,totalflow_mic,totalflow_mac
def draw_img(signal,position,name):
    x=np.arange(len(signal))
    global start_1 
    global end_1
    plt.clf()
    plt.plot(x,signal)
    plt.savefig(position+name+'_'+str(start_1)+'_'+str(end_1)+'.png')
...
def draw_roiline19(path1,path2, qian, hou,fs):  #与16相比再增加两个位置眼睑部位
    # folder = os.path.exists("D:/dataset/micro_datatset/test_casme_face8/"+path2)
    # pathp = "D:/dataset/micro_datatset/test_casme1_face8/" + path2   #存储的位置
    # print(pathp)
    ...
    if 'Thumbs.db' in fileList1:
        fileList1.remove('Thumbs.db')
    global img_save
    img_save = "./images/"+path2
        ...
        for i in fileList:
            ...
            if(k>=start):
                if(k==start):
                    ...
                    landmark0 = tu_landmarks(gray, img_rd, landmark0, frame_shang, frame_left,frame_xia-frame_shang,frame_right-frame_left,img_size)  # 对人脸68个点的定位
                    #相对与新图片的68点的位置。
                    ...
                    # cv2.imwrite(img_save+'/roi.jpg',img_rd)
                    ...
                    if (True):
                        ...
                        # prevgray_roi4 = gray_roi4
                        # prevgray_roi5 = gray_roi5
                        # prevgray_roi6 = gray_roi6
                        # prevgray_roi7 = gray_roi7
            ...
            if (k  == end):
                ...
                # print(k)
                a=1
                totalflow,totalflowmic,totalflowmac=proce2(flow1_total,1.4,1.8,"left_eye",0,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow1_total1,1.4,1.8,"left_eye",1,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow1_total2,1.4,1.8,"left_eye",2,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow1_total3,1.4,1.8,"left_eye",3,k,a,totalflow,totalflowmic,totalflowmac)

                totalflow,totalflowmic,totalflowmac=proce2(flow3_total,1.4,1.8,"right_eye",0,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow3_total1,1.4,1.8,"right_eye",1,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow3_total2,1.4,1.8,"right_eye",2,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow3_total3,1.4,1.8,"right_eye",3,k,a,totalflow,totalflowmic,totalflowmac)

                totalflow,totalflowmic,totalflowmac=proce2(flow4_total,1.4,1.85,"mouth",0,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow4_total1,1.4,1.85,"mouth",1,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow4_total2,1.4,1.85,"mouth",2,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow4_total3,1.4,1.85,"mouth",3,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow4_total4,1.4,1.85,"mouth",4,k,a,totalflow,totalflowmic,totalflowmac)
                # totalflow,totalflowmic,totalflowmac=proce2(flow4_total5,1.4,1.85,"mouth",5,k,a,totalflow,totalflowmic,totalflowmac)

                # totalflow, totalflowmic, totalflowmac = proce2(flow5_total1, 1.4, 2.1, "nose", 1, k, a, totalflow,totalflowmic, totalflowmac)
                # totalflow, totalflowmic, totalflowmac = proce2(flow5_total2, 1.4, 2.1, "nose", 2, k, a, totalflow,totalflowmic, totalflowmac)
                #
                ...
                # totalflowmic_1=np.array(nms2(totalflowmic,0.2))
                # totalflowmac_1=np.array(nms2(totalflowmac,0.2))
                ...
                # print(label_vio)
                # label_vio=np.vstack((label_vio,totalflowmac_1))
                break
    print("全部：")
    # print(label_vio)
