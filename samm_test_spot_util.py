...
def crop_picture(img_rd,size):
    ...
    #     cv2.circle(img_rd, (x,y), 1, (255, 0, 0), 2)
    #两个眼角的位置
    ...
    gezi=int((right[0,0]-left[0,0])/2) #左右眼角宽度的一半
...
def get_roi(flow,percent):
    ...
    return x,y
# 返回图像的68个标定点
def tu_landmarks(gray,img_rd,landmark0,frame_shang,frame_left,w,h):
    faces = detector(gray, 0)
    if(len(faces)==0):
        landmark0[:, 0]=(landmark0[:,0]-frame_left)*(320/w)
        landmark0[:, 1]=(landmark0[:,1]-frame_shang)*(320/h)
...
def fenxi1(flow_total,imf_sum1,yuzhi1,yuzhi2): #使用寻找峰的方法 分解前的信号 分解后的信号 1.3 1.5
    flow_total = np.array(flow_total)
    low=np.min(flow_total)    #找到最小值
    flow_total=flow_total-low     #从零开始
    flow_total_fenxi=[]
    for j in range(len(flow_total)):#找到大于较小阈值 j表示帧数
        if(flow_total[j]>=yuzhi1):
    ...
    if(len(flow_total_fenxi)>0): #对经过第一步筛选的，帧相邻的连在一起
        ...
        for i in range(len(flow_total_fenxi)):
            ...
            #阈值高且3帧内相邻的连续片段设为表情片段
            if(flow_total_fenxi[i]-end>=3):
                flow_total_pp.append([start,end])
                start = flow_total_fenxi[i]
            end=flow_total_fenxi[i]
        flow_total_pp.append([start, end])
    ...
    for i in range(len(flow1_total_fenxi)):
        ...
        if(st_arglow<start):
            for j in range(start-1,-1,-1):
                if(flow1_total_edm[j]-st_low<0.33*(high-st_low)):# 0.33？
                    start=j
                    break
            ...
            if(flow1_total_edm[start]-flow1_total_edm[aa]>0.3): #往左找大于0.3的断崖
                start=aa+1
        if (en_arglow > end):
...
def proce2(flow1_total,yuzhi1,yuzhi2,position,xuhao,k,a,totalflow,totalflow_mic,totalflow_mac):
    ...
    filter_threshold=5
    flow1_total_edm1 = sim_filter.filt(flow1_total[a:-a], 1, filter_threshold, 30)  # 滤波
    ...
    #上面两个变量得到经验模态分量的后两个 和 前两个分量之和
    flow1_total_fenxi = fenxi1(flow1_total_edm1,imf_sum1,yuzhi1,yuzhi2)  #得到了分析结果
    # flow1_total_fenxi = fenxi1(flow1_total_edm2,imf_sum1,yuzhi1,yuzhi2)
    ...
    for i in range(len(totalflow)):
        ...
        for j in range(1,len(hh)):
            ...
            if(iou>threshold):#SAMM0.34  CASME 0.29   #如果重复率比较高就
                ...
                # hh[j, 1]=(hh[j, 1]+totalflow[i, 1])/2
                # hh[j, 0]=(hh[j, 0]+totalflow[i, 0])/2
        if(new ==1):
            hh=np.vstack((hh, [[totalflow[i, 0], totalflow[i, 1]]]))
    return hh
...
def draw_roiline19(path1,path2, qian, hou,fs):  #与16相比再增加两个位置眼睑部位
    folder = os.path.exists("./SAMM_test/"+path2)
    pathp = "./SAMM_test/" + path2   #存储的位置
    global img_save
    ...
    fileList1.sort(key=lambda x: x.split('_')[-1])  #对提取的图片排序
    fileList = []
    l=0
    for i in fileList1:
        if(l%fs==0):
        ...
        for i in fileList:
            ...
            if(k>=start):
                if(k==start):
                    ...
                    flow5_total1 = [[0, 0]]
                    flow5_total2 = [[0, 0]]
                    flow2_total1 = [[0, 0]] #鼻尖区域的光流平均值
                    flow6_total= [[0, 0]]
                    flow7_total= [[0, 0]]


                    img_rd = cv2.imread(path + i)  # D:/face_image_test/EP07_04/
                    landmark0, img_rd, frame_shang, frame_xia, frame_left, frame_right = crop_picture(img_rd,320) #img_rd即为图片
                    #记录框的位置，上下左右在整个图片中的坐标，和68点的位置。img_rd是被裁减之后的面部位置，并resize到320*320


                    gray = cv2.cvtColor(img_rd, cv2.COLOR_RGB2GRAY)  #变成灰度图
                    landmark0 = tu_landmarks(gray, img_rd, landmark0, frame_shang, frame_left,frame_xia-frame_shang,frame_right-frame_left)  # 对人脸68个点的定位
                    #相对与新图片的68点的位置。
                    for index,landmark in enumerate(landmark0):
                        x = landmark.A[0][0]
                        y = landmark.A[0][1]
                        cv2.putText(img_rd, str(index), (x,y), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 0, 255), 1)
                        cv2.circle(img_rd, (x,y), 1, (255, 0, 0), 2)
                    round1 = 0
                    ...
                    cv2.rectangle(img_rd, (roi1_left-5, roi1_low - 15),
                                  (roil_right, roi1_high + 5), (0, 255, 255), 1)
                    prevgray_roi1 = gray[max(0,(roi1_low - 15)):roi1_high + 5, roi1_left-5:roil_right]#修改
                    ...
                    cv2.rectangle(img_rd, (roi3_left, roi3_low - 15),
                                  (roi3_right+5, roi3_high + 5), (0, 255, 255), 1)
                    ...
                    cv2.rectangle(img_rd, (roi4_left-20, roi4_low - 15),
                                  (roi4_right+20, roi4_high + 10), (0, 255, 255), 1)
                    ...
                    if (True):
                        ...
                        x1, y1 = get_roi(flow2[15:-10, 5:-5, :], 0.7) #这个15，-10, 5，-5
                        ...
                        # print(roi1_sma)
                        a, b = get_roi(flow3[round1:-round1, round1:-round1, :], 0.3) #取0.3
                        a1, b1 = get_roi(
            ...
            if (k  == end):
                ...
                #
                # totalflow, totalflowmic, totalflowmac = proce2(flow6_total, 1.8, 2.5, "nose", 2, k, a, totalflow,totalflowmic, totalflowmac)
                # totalflow, totalflowmic, totalflowmac = proce2(flow7_total, 1.8, 2.5, "nose", 2, k, a, totalflow,totalflowmic, totalflowmac)
    ...
    for i in range(len(label_vio)):
        if(label_vio[i,1]-label_vio[i,0]>=10):
